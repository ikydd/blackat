const mockPacks = require("@repo/testing-fixtures/nrdb/packs.json");
const mockCycles = require("@repo/testing-fixtures/nrdb/cycles.json");
...
describe("process factions", () => {
  ...
  it("does not output campaign pack from terminal directive", () => {
    const output = process(mockPacks, mockCycles);
    const terminalDirective = output.find(
      ({ code }) => code === "terminal-directive"
    );
