...
const isOfficialCard = (pack_code, packs) =>
  packs.find(({ code }) => code === pack_code).official;
const isRotatedCard = (pack_code, packs) =>
  packs.find(({ code }) => code === pack_code).rotated;
const isBannedCard = (code, bannedCards) =>
  bannedCards.some((id) => code === id);
...
const process = ({ imageUrlTemplate, data: cards }, cycles, { data: mwl }) => {
  const includedPacks = cycles.reduce(
