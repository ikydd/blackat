...
const imgFolder = path.join(__dirname, "..", "..", "output", "cards");
...
const importCards = async (packs) => {
  ...
  if (!ajv.validate(cardsSchema(), cardApiData)) {
    throw new Error(
      `Cards data does not match schema: ${JSON.stringify(ajv.errors)}`
    );
  }
  const mwlApiData = await request(getApiUrl("/mwl"));
  if (!ajv.validate(mwlSchema(), mwlApiData)) {
    throw new Error(
      `MWL data does not match schema: ${JSON.stringify(ajv.errors)}`
    );
  }
