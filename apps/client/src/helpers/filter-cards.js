...
const hasMatchingFactions = (card, factions) =>
  !factions.length || factions.includes(card.faction);

const hasMatchingType = (card, types) =>
  !types.length || types.includes(card.type);

const hasMatchingSubtypes = (card, subtypes) =>
  !subtypes.length ||
  subtypes.some(
    (subtype) => card.keywords && card.keywords.search(subtype) !== -1
  );

const isInMatchingPack = (card, packs) =>
  !packs.length || packs.includes(card.pack);
...
const confersAgendaPoints = (card) =>
  ...
  !!(card.text && card.text.search(/as an agenda/) !== -1);
...
export const filterCards = (cards, filters) =>
  cards
    ...
    .filter((card, index, all) =>
      isPreferredArt(card, filters.originalArt, index, all)
    );

export const filterBySide = (cards, side) =>
  cards.filter((card) => isFromCorrectSide(card, side));
