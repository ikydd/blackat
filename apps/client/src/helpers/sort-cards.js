...
const compareProperty = (prop, a, b, order = "asc") =>
  compare(a[prop], b[prop], order);
...
const compareByMultipleProps = (list, categories, a, b) =>
  list.reduce(
    (comparison, property) =>
      compareByProp(property, categories, a, b, comparison),
    0
  );
...
const sortCards = (
  ...
  const sortedCards = multipleSortMethods
    ? cards.sort((a, b) =>
        compareByMultipleProps(multipleSortMethods, categories, a, b)
      )
