...
const customGroup = ({
  ...
  const groupCode =
    typeof card[sortProp] === "undefined" ? unknown : card[sortProp];
  const name =
    typeof groupCode === "number" && groupCode >= max ? `${max}+` : groupCode;
...
const sortSections = (sections, sort) => {
  switch (sort) {
    ...
    default:
      return sections.sort((a, b) =>
        compare({ a: a.info.code, b: b.info.code })
      );
  }
};
