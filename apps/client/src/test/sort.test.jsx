...
describe("Sort", () => {
  ...
  it("sorts by faction by default", async () => {
    api.setData(
      "cards",
      await loadFile("api/faction-sort/faction-runner.json")
    );
    render(<App />);
  ...
  describe.each(sortMethods)(
    ...
    ({ code, side = "runner", file, matches, headers }) => {
      it("sorts correctly", async () => {
        api.setData("cards", await loadFile(`api/${file}`));
  ...
  it("sorts asc correctly with undefined", async () => {
    api.setData("cards", await loadFile("api/cost-sort/undefined.json"));
    render(<App />);
    await sortBy("cost");
  ...
  it("sorts desc correctly with undefined", async () => {
    api.setData("cards", await loadFile("api/agenda-sort/undefined.json"));
  ...
  it("sorts X to the end when asc", async () => {
    api.setData("cards", await loadFile("api/cost-sort/x.json"));
    render(<App />);
    await sortBy("cost");
  ...
  it("sorts X to the end when desc", async () => {
    api.setData("cards", await loadFile("api/strength-sort/x.json"));
    render(<App side="corp" />);
