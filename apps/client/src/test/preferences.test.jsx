...
const getMockBlades = (mockData) => {
  const [{ imagesrc: blade1 }, , { imagesrc: blade2 }, , { imagesrc: blade3 }] =
    mockData;
...
describe("Preferences filters", () => {
  ...
  describe("Prefer Original Art", () => {
    it("does not filter any out when they are not adjacent", async () => {
      const mockData = await loadFile("api/updated.json");
    ...
    it.skip("selects cards correctly when some are filtered out", async () => {
      const mockData = await loadFile("api/updated.json");
  ...
  describe("Official Packs Only", () => {
    it("filters cards correctly", async () => {
      const mockData = await loadFile("api/official.json");
      api.setData("cards", mockData);
      render(<App />);
  ...
  describe("Pack Legality", () => {
    it("filters rotated cards correctly", async () => {
      const mockData = await loadFile("api/rotated.json");
      api.setData("cards", mockData);
      render(<App />);
    ...
    it("filters banned cards correctly", async () => {
      const mockData = await loadFile("api/banned.json");
      api.setData("cards", mockData);
      render(<App />);
