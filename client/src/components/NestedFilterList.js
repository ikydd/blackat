...
const NestedFilterList = ({
  ...
  const changeGroup = (group) => ({ target: { checked }}) => {
    onGroupChange(group, checked);
  }

  const changeSubitem = (item) => ({ target: { checked }}) => {
  ...
  const generateFilters = (options) => options.map((group) => {
    const subFilters = group.items.length > 1 &&
      <div className="filter-group-items">
        {group.items.map((item) => <FilterItem key={item.code} item={item} keyword={keyword} onChange={changeSubitem} />)}
      </div>;

    return <div className="filter-group" key={group.code}>
