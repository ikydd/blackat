...
const countCheckboxes = (options) => options.reduce((total, group) => (group.items.length > 1 ? total + group.items.length : total) + 1, 0);
...
describe('NestedFilterList', () => {
  ...
  describe('Options', () => {
    ...
    it('calls the callback when the group is checked', async () => {
      ...
      const { findByLabelText } = render(<NestedFilterList options={options} onGroupChange={groupCb} />);
    ...
    it('calls the callback when the group is unchecked', async () => {
      const groupCb = jest.fn();
      const { findByLabelText } = render(<NestedFilterList options={optionsSelected} onGroupChange={groupCb} />);
      const group = await findByLabelText('Bar');
      fireEvent.click(group);
    ...
    it('calls the callback when an item is checked', async () => {
      const cb = jest.fn();
      const { findByLabelText } = render(<NestedFilterList options={options} onSubitemChange={cb} />);
  ...
  describe('visibility toggle', () => {
    ...
    it('shows options when hidden and heading is clicked', async () => {
      const { findAllByRole, getByRole } = render(<NestedFilterList options={options} hidden={true} />);
